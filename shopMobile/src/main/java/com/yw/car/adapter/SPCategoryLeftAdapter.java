...
public class SPCategoryLeftAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(mContext).inflate(com.yw.car.R.layout.category_left_item, parent, false);
            holder.nameTxtv = (TextView) convertView.findViewById(com.yw.car.R.id.catelogy_left_item_name);
            holder.highLinev = convertView.findViewById(com.yw.car.R.id.high_linev);
            convertView.setTag(holder);
        } else {
            holder = (ViewHolder) convertView.getTag();
        }
        if (curSelectRow == position) {         //选中的行:背景白色,否则灰色
            convertView.setBackgroundColor(mContext.getResources().getColor(com.yw.car.R.color.bg_activity));
            holder.nameTxtv.setTextColor(mContext.getResources().getColor(com.yw.car.R.color.light_red));
            holder.highLinev.setVisibility(View.VISIBLE);
        } else {
            convertView.setBackgroundColor(mContext.getResources().getColor(com.yw.car.R.color.bg_view));
