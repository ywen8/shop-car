...
import com.yw.car.activity.common.SPTextAreaViewActivity_;
import com.yw.car.activity.person.address.SPConsigneeAddressListActivity_;
import com.yw.car.activity.person.order.SPOrderCouponListActivity_;
import com.yw.car.activity.person.order.SPOrderListActivity_;
import com.yw.car.adapter.SPConfirmOrderAdapter;
...
import com.yw.car.widget.KeyboardDialog;
import com.yw.car.widget.SPArrowRowView;
import com.yw.car.widget.SPRecyclerviewMarginLineDivider;
...
@EActivity(com.yw.car.R.layout.order_confirm_order)
public class SPConfirmOrderActivity extends SPBaseActivity implements SPConfirmOrderAdapter.SPConfirmOrderListener, View.OnClickListener {
    ...
    @ViewById(com.yw.car.R.id.confirm_ll)
    FrameLayout confirmLl;

    @ViewById(com.yw.car.R.id.store_listv)
    RecyclerView mRecyclerView;

    @ViewById(com.yw.car.R.id.payfee_txtv)
    TextView payfeeTxtv;

    int points;                                        //使用的金豆
    int integral;                                      //金豆商品使用的金豆
    float balance;                                     //使用的余额
    View mHeaderView;
    View mFooterView;
    private String pwd;                                //密码
    TextView pointTxtv;                                //现有金豆
    TextView hasPointTv;
    ...
    JSONObject userinfoJson;                           //用户信息(金豆,余额)
    ...
    public void initSubViews() {
        ...
        mHeaderView = LayoutInflater.from(this).inflate(com.yw.car.R.layout.order_confirm_order_header, null);
        mFooterView = LayoutInflater.from(this).inflate(com.yw.car.R.layout.order_confirm_order_footer, null);
        ...
        address_rl = (RelativeLayout) mHeaderView.findViewById(com.yw.car.R.id.address_rl);
        addressRl = (RelativeLayout) mHeaderView.findViewById(com.yw.car.R.id.address_consignee_rl);
        addressTv = (TextView) mHeaderView.findViewById(com.yw.car.R.id.address_consignee_tv);
        mConsigneeTxtv = (TextView) mHeaderView.findViewById(com.yw.car.R.id.order_consignee_txtv);
        mAddressTxtv = (TextView) mHeaderView.findViewById(com.yw.car.R.id.order_address_txtv);
        mAddressTxtv.setVisibility(View.GONE);
        RelativeLayout orderPointRl = (RelativeLayout) mFooterView.findViewById(com.yw.car.R.id.order_point_rl);
        RelativeLayout hasPointRl = (RelativeLayout) mFooterView.findViewById(com.yw.car.R.id.has_point_rl);
        hasPointTv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_has_point);
        feeCutfeeTv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_cutfee_txtv);
        feeGoodsFeeTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_goodsfee_txtv);
        feeShoppingTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_shopping_txtv);
        RelativeLayout couponRl = (RelativeLayout) mFooterView.findViewById(com.yw.car.R.id.coupon_rl);
        feeCouponTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_coupon_txtv);
        RelativeLayout pointRl = (RelativeLayout) mFooterView.findViewById(com.yw.car.R.id.point_rl);
        feePointTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_point_txtv);
        feeBalanceTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_balance_txtv);
        feeAmountTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.fee_amount_txtv);
        buyTimeTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.buy_time_txtv);
        pointEt = (EditText) mFooterView.findViewById(com.yw.car.R.id.order_point_et);
        balanceEt = (EditText) mFooterView.findViewById(com.yw.car.R.id.order_balance_et);
        pointSth = (SwitchButton) mFooterView.findViewById(com.yw.car.R.id.order_point_sth);
        bananceSth = (SwitchButton) mFooterView.findViewById(com.yw.car.R.id.order_balance_sth);
        pointEt.setEnabled(false);
        balanceEt.setEnabled(false);
        pointSth.setClickable(false);
        bananceSth.setClickable(false);
        balanceTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.order_balance_txtv);
        pointTxtv = (TextView) mFooterView.findViewById(com.yw.car.R.id.order_point_txtv);
        orderInvoceArv = (SPArrowRowView) mFooterView.findViewById(com.yw.car.R.id.order_invoce_aview);
        orderInvoceArv.setSubText(getString(com.yw.car.R.string.invoce_message));
        if (isIntegralGood) {
    ...
    public void refreshData() {
        ...
        if (isIntegralGood) {            //金豆商品立即兑换订单信息
    ...
    public void refreshView() {
        try {
            ...
            if (userinfoJson != null) {
                if (userinfoJson.has("pay_points")) {        //金豆
                    ...
                    pointTxtv.setText("当前可用金豆" + p + "(" + pointRate + "金豆抵扣1元)");
                }
            ...
            if (amountDict != null) {
                if (amountDict.has("store_list_pay_info") && SPUtils.velidateJSONObject(amountDict.get("store_list_pay_info"))) {
                    JSONObject storeListInfoObj = amountDict.getJSONObject("store_list_pay_info");
                    for (SPStore spStore : cacheStores) {
                        if (storeListInfoObj.has(spStore.getStoreId() + "")) {
                            JSONObject storeInfoObj = storeListInfoObj.getJSONObject(spStore.getStoreId() + "");
                            if (!storeInfoObj.isNull("order_prom_title"))            //商品活动信息
                                spStore.setPromTitle(storeInfoObj.getString("order_prom_title"));
                            if (!storeInfoObj.isNull("shipping_price")) {            //商品物流信息
                                double shipfee = storeInfoObj.getDouble("shipping_price");
                                if (shipfee <= 0)
    ...
    public void refreshTotalFee() {
        try {
            if (amountDict != null) {
                ...
                if (amountDict.has("integral_money"))           //金豆支付
                ...
                if (isIntegralGood && amountDict.has("pay_points"))           //当前使用的金豆
                    hasPointTv.setText(amountDict.getInt("pay_points") + "");
                ...
                if (amountDict.has("order_amount")) {
                    ...
                    payablesSpanStr.setSpan(new ForegroundColorSpan(getResources().getColor(com.yw.car.R.color.light_red)), startIndex,
            ...
            if (amountDict != null && amountDict.has("order_amount")) {
                ...
                if (isIntegralGood && integral <= 0) {            //当金豆商品兑换金豆不足时展示的商品总额和支付金额
            ...
            totalFeeSpanStr.setSpan(new ForegroundColorSpan(getResources().getColor(com.yw.car.R.color.light_red)), rmbStartIndex,
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case com.yw.car.R.id.address_rl:           //收货地址
                ...
                break;
            case com.yw.car.R.id.order_invoce_aview:            //发票
    ...
    private void isInputPwd() {
        ...
        if (hasPoint || hasBanance || integral > 0) {         //使用了金豆(余额)支付
    ...
    public RequestParams getRequestParameter(int type) {
        ...
        try {
            if (isIntegralGood) {
                ...
                if (orderInvoceArv != null && !invoce.isEmpty() && !invoce.equals(getString(com.yw.car.R.string.invoce_message))) {
                ...
                params.put("pay_points", points);                //使用金豆
                params.put("user_money", balance);               //使用余额
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        SPShopRequest.getOrderAmountWithMasterOrderSN(masterOrderSN, new SPSuccessListener() {
            ...
            public void onRespone(String msg, Object response) {
                if (response != null && Double.valueOf(response.toString()) > 0) {
                    ...
                    startupOrderList();      //支付金额为0时,说明是金豆或余额抵扣,进入订单列表页面
                }
            }
        }, new SPFailuredListener(SPConfirmOrderActivity.this) {
            @Override
